...
import { extractOntologyPrompt_es } from '@/lib/prompts';
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { intakeId, freeNotes, existingSystems } = requestSchema.parse(body);

    const requestHash = hashRequest({ freeNotes, existingSystems });

    const existingLog = await prisma.actionLog.findFirst({
      where: {
        intakeId,
        actionName: 'extractOntology',
        requestHash,
        status: 'ok',
      },
    ...
    const prompt = extractOntologyPrompt_es(freeNotes, existingSystems);
    const response = await callGemini(prompt, 'fast');
    ...
    console.error('extractOntology error:', error);
